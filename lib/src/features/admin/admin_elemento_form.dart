// lib/src/features/admin/admin_elemento_form.dart
import 'dart:io'; 
...
class _AdminElementoFormScreenState extends State<AdminElementoFormScreen> {
  ...
  // ... (Lógica _handlePickImage y _handleUploadImage idéntica a propuesta_edit_screen) ...
  ...
  Future<void> _handleGuardar() async {
    ...
    if (_uploadedImageUrl == null) {
      SnackBarHelper.showTopSnackBar(ScaffoldMessenger.of(context), 'Por favor, sube una imagen de portada.', isError: true);
      return;
    }
  ...
  Widget build(BuildContext context) {
    final String tituloPantalla = widget.isEditMode ? 'Editar Elemento' : 'Crear Elemento Oficial';
    final String botonGuardar = widget.isEditMode ? 'Guardar Cambios' : 'Crear Elemento';

    return Scaffold(
      appBar: AppBar(
        title: Text(tituloPantalla, style: Theme.of(context).textTheme.titleLarge),
  ...
  Widget _buildImageUploader() {
    ...
    if (_pickedImage != null) {
      ...
      content = CachedNetworkImage(
        ...
        errorWidget: (c, u, e) => Icon(Icons.image_not_supported, size: 48, color: Colors.grey[600]),
      );
    } else {
