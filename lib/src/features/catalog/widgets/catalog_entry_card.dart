...
import 'dart:math'; // <-- ¡NUEVA IMPORTACIÓN! Para 'max'
...
class _CatalogEntryCardState extends State<CatalogEntryCard> {
  ...
  void _actualizarEstadoEntrada(CatalogoEntrada nuevaEntrada) {
    ...
    _episodiosPorTemporada = _parseEpisodiosPorTemporada(_entrada.elementoEpisodiosPorTemporada);
  ...
  Future<void> _handleUpdateEstado(String nuevoEstado) async {
    ...
    try {
      ...
      if (mounted) {
        ...
        setState(() {
          _actualizarEstadoEntrada(entradaActualizada); 
          _isLoading = false;
        });
        SnackBarHelper.showTopSnackBar(msgContext, 'Estado actualizado.', isError: false);
      }
    } catch (e) {
      if (mounted) {
        setState(() { _isLoading = false; });
        SnackBarHelper.showTopSnackBar(msgContext, 'Error al actualizar: ${e.toString().replaceFirst("Exception: ", "")}', isError: true);
      }
    }
  }
  ...
  Future<void> _handleUpdateProgreso({
    ...
    if (esGuardadoManual) {
      ...
      if (capitulo == _entrada.capituloActual && pagina == _entrada.paginaActual) {
        return;
      }
    }
    ...
    if (tipo == 'serie' && temporada != null && unidad != null && totalTemps > 0) {
      if (temporada == totalTemps && unidad == _episodiosPorTemporada[totalTemps - 1]) {
        autoTerminado = true;
      }
    ...
    } else if ((tipo == 'anime' || tipo == 'manga') && unidad != null && totalEpsAnime > 0) {
      if (unidad == totalEpsAnime) {
        autoTerminado = true;
      }
    }
    
    if (autoTerminado && _entrada.estadoPersonal != EstadoPersonal.TERMINADO.apiValue) {
    ...
    try {
      final CatalogoEntrada entradaActualizada =
          await _catalogService.updateElementoDelCatalogo(
        _entrada.elementoId,
        estado: estadoFinal, // Envía "TERMINADO" si se autocompletó
      ...
      if (mounted) {
        ...
        if (esGuardadoManual) {
          SnackBarHelper.showTopSnackBar(msgContext, 'Progreso guardado.', isError: false);
        }
      }
    } catch (e) {
      if (mounted) {
  ...
  double get _progresoValue {
    ...
    try {
      if (tipo == 'serie') {
        ...
        final totalEps = _episodiosPorTemporada.fold<int>(0, (prev, e) => prev + e);
        if (totalEps == 0) return 0.0;
        
        int currentEps = 0;
        int tempActual = (_entrada.temporadaActual ?? 1).clamp(1, _episodiosPorTemporada.length);
  ...
  Widget build(BuildContext context) {
    ...
    final tipo = _entrada.elementoTipoNombre.toLowerCase();
    final bool mostrarProgresoUI = tipo == 'serie' || tipo == 'libro' || tipo == 'anime' || tipo == 'manga';
  ...
  Widget _buildProgresoSerie(BuildContext context, bool isEditable) {
    final int tempActual = (_entrada.temporadaActual ?? 1).clamp(1, max(1, _episodiosPorTemporada.length));
    final int epTotalTempActual = (tempActual - 1 < _episodiosPorTemporada.length) 
        ? _episodiosPorTemporada[tempActual - 1] 
        : 0;
    final int epActual = (_entrada.unidadActual ?? 0).clamp(0, max(0, epTotalTempActual));
    
    // Generamos las listas para los Dropdowns
    final List<int> temporadas = List.generate(_episodiosPorTemporada.length, (i) => i + 1);
    final List<int> episodios = List.generate(epTotalTempActual + 1, (i) => i); // +1 para incluir el 0
      ...
      children: [
        ...
        Expanded(
          child: DropdownButtonFormField<int>(
            ...
            items: temporadas.map((t) => DropdownMenuItem(value: t, child: Text('T$t'))).toList(),
        ...
        Expanded(
          child: DropdownButtonFormField<int>(
            ...
            items: episodios.map((e) => DropdownMenuItem(value: e, child: Text('Ep $e'))).toList(),
            onChanged: isEditable ? (val) {
  ...
  Widget _buildProgresoUnidad(BuildContext context, bool isEditable, String tipo) {
    ...
    return Row(
      children: [
        ...
        Text('$uniActual / $uniTotal', style: Theme.of(context).textTheme.bodyLarge),
        IconButton(
          icon: const Icon(Icons.add_circle_outline),
          iconSize: 20,
          color: isEditable && uniActual < uniTotal ? Colors.white : Colors.grey[700],
  ...
  void _handleUpdateProgresoLibro() {
    final int cap = int.tryParse(_capituloController.text) ?? _entrada.capituloActual ?? 0;
    final int pag = int.tryParse(_paginaController.text) ?? _entrada.paginaActual ?? 0;
